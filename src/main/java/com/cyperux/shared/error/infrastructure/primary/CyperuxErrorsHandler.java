...
import com.cyperux.shared.enumeration.domain.Enums;
import com.cyperux.shared.error.domain.ErrorKey;
import com.cyperux.shared.error.domain.CyperuxException;

@ControllerAdvice
@Order(Ordered.LOWEST_PRECEDENCE - 1000)
class CyperuxErrorsHandler {

  private static final Logger log = LoggerFactory.getLogger(CyperuxErrorsHandler.class);
  private static final String MESSAGES_PREFIX = "error.";

  private final MessageSource messages;

  public CyperuxErrorsHandler(@Qualifier("applicationErrorMessageSource") MessageSource messages) {
    Locale.setDefault(Locale.ENGLISH);

    this.messages = messages;
  }

  @ExceptionHandler(CyperuxException.class)
  ProblemDetail handleCyperuxException(CyperuxException exception) {
  ...
  private String buildDetail(CyperuxException exception) {
  ...
  private void logException(CyperuxException exception, HttpStatus status) {
